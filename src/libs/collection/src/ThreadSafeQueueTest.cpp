...
using namespace libs::collection;
...
void popFromOneQueueAndPushToOther(ThreadSafeQueue<std::string>& inputQueue, ThreadSafeQueue<std::string>& outputQueue)
{
...
public:
    ...
    const std::vector<std::string> data{100, element1};
};
...
TEST_F(ThreadSafeQueueTest, pushElementsWithMultipleThreads_shouldNotDeadlock)
{
    ...
    std::thread t1(&pushElements, data, std::ref(queue));
    std::thread t2(&pushElements, data, std::ref(queue));
    std::thread t3(&pushElements, data, std::ref(queue));
    t1.join();
    t2.join();
    t3.join();
}
...
{
    ThreadSafeQueue<std::string> queueWithElements{data};
...
{
    ThreadSafeQueue<std::string> inputQueue{data};
    ...
    ASSERT_EQ(resultData.size(), data.size());
}
